    ...
    try:
        ...
        if expect_failure:
            if final_state == "FAILED":
                print(
                    f"✅ {test_name}: Job {job_id} failed as expected (exit code test)"
                )
                return True
            else:
                print(
                    f"⚠️ {test_name}: Job {job_id} should have failed but got state: {final_state}"
                )
                return False
        else:
            if final_state == "COMPLETED":
                ...
                print(
                    f"⏱️ {test_name}: Job {job_id} still running after {max_wait}s, cancelling..."
                )
                try:
